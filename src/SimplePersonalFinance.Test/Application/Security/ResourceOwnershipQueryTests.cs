...
public class ResourceOwnershipQueryTests
{
    ...
    public async Task GetAccount_WhenOwnedByAnotherUser_ShouldReturnNotFoundSemantics()
    {
        ...
        uow.Setup(unit => unit.Accounts).Returns(accountRepository.Object);
        currentUser.SetupGet(user => user.UserId).Returns(currentUserId);

        var handler = new GetAccountByIdQueryHandler(uow.Object, currentUser.Object);
    ...
    public async Task GetBudget_WhenOwnedByAnotherUser_ShouldReturnNotFoundSemantics()
    {
        ...
        uow.Setup(unit => unit.Budgets).Returns(budgetRepository.Object);
        currentUser.SetupGet(user => user.UserId).Returns(currentUserId);

        var handler = new GetBudgetByIdQueryHandler(uow.Object, currentUser.Object);
    ...
    public async Task GetTransaction_WhenAccountOwnedByAnotherUser_ShouldReturnNotFoundSemantics()
    {
        ...
        accountRepository.Setup(repository => repository.GetByIdAsync(accountId)).ReturnsAsync(account);
        uow.Setup(unit => unit.Transactions).Returns(transactionRepository.Object);
        uow.Setup(unit => unit.Accounts).Returns(accountRepository.Object);
        currentUser.SetupGet(user => user.UserId).Returns(currentUserId);

        var handler = new GetTransactionByIdQueryHandler(uow.Object, currentUser.Object);
